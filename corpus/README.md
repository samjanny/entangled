...
- **Image** (`W_IMAGE_*`): four of the seven codes are covered by vectors 240-245 (`W_IMAGE_HASH_MISMATCH`, `W_IMAGE_CONTENT_TYPE`, `W_IMAGE_DIMENSIONS`, and `W_IMAGE_DECODE_FAILED` in both its undecodable-bytes and APNG-animation forms), via the `context.image_responses` / `expected.image_outcomes` schema described above. The remaining three stay deferred, each for a concrete reason. `W_IMAGE_OVERSIZE` needs a response body over 2 MiB whose every other property is valid; committing multi-megabyte binaries weighs every checkout and the rule is a single byte-length comparison, so it stays at the client's unit-test layer for now. `W_IMAGE_BUDGET` needs more than 16 megapixels of fully decodable PNG across one document: real pixel data at that scale requires actual DEFLATE compression, and a compressor's output is not bit-stable across zlib builds, which would break the corpus's byte-for-byte reproducibility; a future tranche can hand-roll a fixed-Huffman zero-run stream (deterministic by construction) to lift this. `W_IMAGE_FETCH_FAILED` is a transport-level failure and shares the deferred Stage 1 transport schema extension. An animated-WebP vector is likewise deferred: a well-formed animated WebP requires a hand-built VP8/VP8L bitstream, and the animation-rejection policy is already pinned format-independently by the APNG vector 241 plus the client unit tests.
...
The image-response extension (`context.image_responses` plus `expected.image_outcomes`) landed with vectors 240-245. The remaining vector-schema extensions (transport metadata, a general expected-warnings array for canary/historical warnings, multi-manifest histories) are deferred to a future tranche. The current corpus exercises every diagnostic code reachable within the existing schema, except `E_SIG_MALFORMED` (not vector-constructible as documented above).
