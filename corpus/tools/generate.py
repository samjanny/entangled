...
def negative_vectors(keys) -> list[dict]:
    ...
    # contribution to the submit body's request_state array is
    # approximately 32 * (2048 value bytes + ~38 envelope bytes) + 31
    # comma separators = ~66783 bytes, well above the state_budget of
    # 53248 bytes defined in §09 ("Submit body budget partition"). Per
    # §07 "Submit budget satisfiability", the manifest is rejected at
    # Stage 5 schema validation as E_SUBMIT_BUDGET with
    # details.component = "state". The manifest is signed correctly and
    # otherwise valid; the satisfiability violation is the only live
    # violation at Stage 5.
    ...
    out.append(vec(
        ...
        description=(
            ...
            "66879 bytes (32 entries * (2048 value bytes + ~39 envelope "
            "bytes) + 31 commas), exceeding the state_budget of 53248 "
            "bytes defined in §09 ('Submit body budget partition') by "
            "13631 bytes. Per §07 'Submit budget satisfiability', "
            "rejected at Stage 5 schema validation as E_SUBMIT_BUDGET "
            "with details.component = 'state'. Manifest is signed "
            "correctly; the satisfiability violation is the only live "
            "Stage 5 violation."
        ),
        spec_refs=["§07", "§09", "§11"],
        verdict="reject",
