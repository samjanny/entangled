...
- **Content `revision` or `content_hash`.** Addressed in v1.0 via `content_root` (§06), `seq` (§02), and the content index mechanism (§02, §09, §10). The manifest's `content_root` field commits to a content index that maps each indexed path to a `(seq, hash)` pair. Indexed paths are frozen at the committed state between ceremonies; `K_runtime`-only forgery and rollback are detected and rejected. Content at unindexed paths remains unprotected, as does content from publishers who do not use `content_root`. Future directions: per-document `content_hash` for publishers who want per-document protection without a manifest-level index; Merkle-tree proof delivery for large-path-count sites where the flat index becomes unwieldy.
