...
- **Request-state suspension on rotation depends on the client observing the rotation.** The §07 rule that suspends transmission of retained `request`-mode state on `K_runtime` rotation (`runtime_superseded`) - the mechanism that prevents a credential planted by a compromised `K_runtime` from outliving the rotation - is triggered only when the client observes a new manifest authorizing a different `K_runtime`. An attacker who also controls the carrier origin (the common case when `K_runtime_priv` is exposed by a server compromise) can keep serving the previous manifest while its canary is still Fresh or Near-expiration, suppressing the rotation observation; submit-time freshness (§10) treats that manifest as fresh and does not force a refresh, so the planted item keeps transmitting. The exposure is not unbounded: it is capped by the previous manifest's canary `next_expected`, since at that deadline the client computes Expired even when the live fetch is withheld (§08, §10), submit-time freshness fails, and submits are blocked. The effective window is therefore at most one canary interval (7 to 30 days, §08), not the up-to-90-day state TTL, and it coincides with the rotation window that bounds `K_runtime` forgery generally. A higher-threat client MAY additionally suspend transmission of a `request`-mode item once the manifest that authorized it has passed its own `next_expected` by the client's clock or the sound `T_lower` reference, without waiting to observe the successor manifest (§07, §10). This limitation concerns `request`-mode state only; `client_only` state is never transmitted (§07).
...
- **No simultaneous multi-origin publication for stateful clients.** A client that maintains publisher history has one current origin per `K_publisher.pub`. Entangled v1 does not support treating distinct origin manifests under the same publisher key as simultaneously current: at equal `canary.issued_at` their distinct signed payloads trigger the §08 conflict rule, while unequal timestamps make the older manifest a downgrade under the publisher-global anti-downgrade rule. Publishers replace an origin through the directional `migration_pointer` flow; before confirmation the successor is verified pending, and after confirmation it replaces the announcing origin for that client's publisher profile (§06, §08, §10). A future multi-origin mechanism is outside the v1 scope.
...
- **Remembered consent does not reprompt on policy widening within the same `(namespace, key, mode)` scope.** Remembered consent under §07 is keyed on `(K_publisher.pub, namespace, key, mode)`. If a publisher changes the `state_policy` for an existing `(namespace, key, mode)` combination to widen `max_size`, widen `max_lifetime`, or change `purpose`, the client MAY commit subsequent set operations under the remembered consent without reprompting the user, provided each new operation stays within the new policy bounds. The protocol does not require fresh consent on policy widening within the same scope. Existing stored entries retain their commit-time bounds and mode per §07 (§07 "Client behavior on policy changes"); the relaxation applies only to new set operations after the policy change. Tighter binding of remembered consent to policy terms is a v1.0 simplification of the consent model and a candidate for tightening in a future protocol version (§07). This simplification concerns the policy bounds only; a change to the stored *value* of a remembered `request`-mode item, which is auto-attached to future submits, is not silent: §07 requires the client to re-affirm or surface a visible notification on such a value change within an unchanged scope (remembered-consent value-change disclosure).

- **Extended runtime-pubkey reuse detection depends on implementation-defined publisher history.** Entangled v1 requires (MUST) clients to reject a manifest whose `runtime_pubkey` matches the *immediately preceding* verified manifest for the same `K_publisher.pub` (§08 `E_CANARY_RUNTIME_REUSE`). Clients that maintain publisher history (the same storage used for anti-downgrade and for cross-session migration history under §10) SHOULD additionally reject a manifest whose `runtime_pubkey` matches *any* previously verified entry in that history (§08), with the same diagnostic distinguished by `details.window_position`. The storage backend is implementation-defined. Stateless clients that retain no publisher history across navigations cannot enforce the extended check and are not non-conforming on that basis: the universal MUST on the immediate-preceding match still applies. The extended check is a defense-in-depth against publisher key-resurrection (re-announcing a `runtime_pubkey` whose private key the ceremony required to be destroyed); it does not defend against a fully-compromised `K_publisher_priv` (an attacker controlling `K_publisher_priv` can simply announce all-fresh runtime keys). The limitation's structural shape - publisher-history-based mitigation enforced where the client retains the necessary state - is identical to the cross-session migration cycle mitigation above (§08, §10).

- **Canary freshness cannot be verified by a client without a reliable current-time reference while offline.** A client without a clock it treats as reliable cannot perform the absolute-time checks defined in §06, §08, and §10 (future-skew bounds on `manifest.updated` and `canary.issued_at`, the `origin.not_after` expiry check, the canary freshness determination distinguishing Fresh, Near-expiration, and Expired). This is a property of the absence of a current-time reference, not a protocol gap: confirming that a canary is currently fresh requires knowing the present moment, and no signed material can supply a value that the client itself does not have access to. Such clients operate in freshness-unverified mode (§10), in which anti-downgrade and structural-validity guarantees remain fully in force, any previously established sound lower-bound expiration determination using `T_lower` remains available (an expiry timestamp strictly less than `T_lower` is definitely past), and the chrome surfaces an explicit, not-easily-dismissible indication that the client cannot place the current time within the canary's claimed validity window. A clock-less verification advances the anti-downgrade floor `T_verified` but not `T_lower`. The future-skew bounds cannot be precisely enforced without a clock and a clock-less client therefore accepts manifests it could not have rejected on those bounds; the residual exposure is bounded by the requirement that the attacker hold `K_publisher_priv` to inflate `issued_at` and poison the anti-downgrade floor, against which a clock-less client has no protocol-provided defense and against which an attacker can mount more direct attacks regardless. This limitation is intrinsic to operating without a current-time reference; a future protocol version cannot resolve it without supplying such a reference.

- **Carrier-layer traffic analysis is not mitigated.** Network-layer anonymity is a carrier property ("not an anonymity layer", above), and the carrier's own padding conceals cell or packet sizes, not the total volume, direction, or timing of a transfer. An observer of the client's access network, or of the path to the carrier entry point, can distinguish flows by volume and timing without reading a byte of payload. Entangled responses are deterministic, byte-exact signed objects served without compression or chunking (§04, §09), so the size of a public document is itself public, and a volume-fingerprinting adversary is in principle better positioned than against the dynamic web. The protocol minimizes its own application-layer fingerprint - the request-header discipline is fixed, there is no content or transfer negotiation, and no application-level handshake, acknowledgment, or heartbeat messages exist (§09) - but it does not hide how much data a session moved or when it moved. Application-level padding and cover traffic are out of scope for v1. Publishers can quantize the served size of JSON documents today, with no wire-format change, because signatures verify over the JCS-canonical form while the wire form is the original non-canonicalized JSON (§04): insignificant inter-token whitespace pads a served document to a fixed size bucket without affecting any signature, and hash bindings hold when computed over the padded bytes. The operator playbook documents the procedure ("Traffic-analysis posture and size bucketing"). Bucketing reduces the uniqueness of per-document sizes; it does not conceal flow existence, timing, or aggregate session volume (§09).
