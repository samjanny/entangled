...
- **Cross-session migration cycle mitigation is opt-in.** Entangled v1 specifies a SHOULD-level migration-history mechanism (§10) by which clients raise user-confirmation friction on cross-session migration cycles for the same publisher identity. Clients that do not implement migration history leave the cross-session ping-pong vector open: a publisher (legitimate or under temporary `K_publisher` compromise) can force re-traversal of an `A ↔ B` migration on every fresh navigation. The mitigation is SHOULD because v1 does not prescribe a publisher-history storage backend beyond what is required for trust-state continuity. Per-flow cycle prevention (visited-origin set) remains MUST and is unaffected (§10).
...
- **No detection of content rollback by an authorized runtime key.** Entangled v1 has no in-document signal that lets a client detect a server serving an older valid version of the same `path` under a still-authorized `K_runtime`. The signed `path` field binds a content document to its location but not to a version or revision; `meta.published_at` is editorial metadata and is not a security signal (§02). A server in possession of a previously-issued signed document for `/some/path` can serve that older instance even after the publisher has published a newer instance for the same path, and the client has no protocol-level basis to distinguish stale from current within the same `K_runtime` authorization window. Operator mitigations (rotate `K_runtime`, publish a new manifest with a later `canary.issued_at`, out-of-band republication of sensitive updates) are the v1 response; an in-document `revision` or `content_hash` mechanism is a v2 candidate noted in `docs/design-decisions.md` (§02, §08, §10).
