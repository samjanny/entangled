...
A client that maintains publisher history (the same publisher-history storage used for anti-downgrade and for cross-session migration history under §10) SHOULD, in addition to the immediate-preceding MUST above, reject a new manifest whose `runtime_pubkey` matches *any* previously verified `runtime_pubkey` for the same `K_publisher.pub` in that history, with the same `E_CANARY_RUNTIME_REUSE` diagnostic (§11). The structured `details` object distinguishes the immediate-preceding match (`window_position = 1`) from a deeper history match (`window_position >= 2`), allowing clients and operators to distinguish the disciplined-rotation violation from the apparent key resurrection. This SHOULD-level extension is the analogue of the §10 SHOULD on cross-session migration history (publisher-history-based mitigation enforced where the client retains the necessary state). A client that does not maintain runtime-pubkey history is not non-conforming; the limitation is documented in §00.
