...
To eliminate this ambiguity, every JSON string field whose value Entangled requires the client to display to the user as text MUST be encoded in Unicode Normalization Form C (NFC) as defined by Unicode Standard Annex #15, evaluated against the Unicode 15.0 Character Database (the pinned baseline; see the assigned-code-point rule below).
...
To keep this determination identical across implementations that link different Unicode Character Database versions, a field subject to NFC MUST contain only code points assigned in the pinned Unicode 15.0 Character Database. A value containing any code point unassigned in Unicode 15.0 - whether permanently reserved, or assigned only in a later Unicode version - is rejected with `E_SCHEMA_FIELD_SYNTAX` (§11), at schema validation, before the NFC check. This assigned-only gate makes the NFC determination version-independent: Unicode normalization stability guarantees that the NFC form of an assigned code point never changes in a later Unicode version, so once code points unassigned at the baseline are rejected, every conforming implementation computes the same NFC result for the remaining baseline-assigned code points regardless of its Character Database version. An implementation MAY use a Character Database newer than 15.0 for the normalization computation provided it still rejects code points unassigned in Unicode 15.0. The baseline is a fixed v1.0 constant; advancing it is a wire-format-relevant change reserved for a future protocol version.
...
## Bidirectional text isolation for displayed values

Unicode normalization does not neutralize bidirectional formatting controls. A publisher-controlled string containing an embedding, override, isolate, or direction-mark control can change the visual ordering of later text if the renderer concatenates that value with adjacent content or client chrome in one bidirectional formatting context. Because Entangled relies on the user reading client-controlled identity, address, canary, and trust indicators correctly, a conforming renderer MUST prevent every displayed publisher-supplied string value from affecting the bidi ordering of any text outside that value.

Each displayed value MUST be placed in a distinct directional-isolation boundary whose base direction is determined from that value (`auto` or first-strong direction), unless the field's grammar fixes a safe base direction such as left-to-right ASCII. The boundary MUST be a renderer or layout primitive that remains in force regardless of bidi control characters contained in the value. Examples include a dedicated UI text element with automatic base direction and sibling isolation, or the equivalent of `unicode-bidi: isolate` plus `direction: auto` in a layout engine. A client MUST NOT concatenate an untrusted value and adjacent text into one plain-text paragraph and rely on paragraph direction alone.

When a rendering backend has no structural isolation primitive and can express isolation only by inserting Unicode control characters, the client MUST first replace every code point whose Unicode 15.0 `Bidi_Control` property is `Yes` (`U+061C`, `U+200E`, `U+200F`, `U+202A` through `U+202E`, and `U+2066` through `U+2069`) with a visible inert escape such as `<U+202E>`. It then wraps the transformed value with `U+2068 FIRST STRONG ISOLATE` and `U+2069 POP DIRECTIONAL ISOLATE`. Wrapping the unmodified value is not sufficient: an embedded `U+2069` could close the wrapper before the value ends, allowing a later override to affect adjacent text.

This is a rendering requirement, not a wire rejection rule. Bidi control characters remain assigned Unicode scalar values and MAY occur in an otherwise conforming NFC string; the client either contains them inside a structural isolation primitive or renders them as visible neutral escapes in the plain-text fallback. Natural right-to-left letters remain permitted and retain normal first-strong layout. Implementations MUST test the boundary with adversarial values containing unterminated overrides and embedded isolate terminators: client-generated sentinel text rendered immediately before and after the value MUST keep the same ordering and direction it has when the publisher value is absent.

The rule applies independently to every value covered by the user-visible-string rule above and to every publisher-derived value placed in client chrome under §10. It does not alter JCS bytes, signature inputs, NFC validation, or the stored value.
