...
3. dissociate from `P` every current, pending, or historical origin association, including successor origins previously adopted under the publisher profile (§10 "Origin migration"). Those origins are no longer associated with the publisher profile of `P`;
...
5. suspend transmission of every retained `request`-mode state item (§07) bound to `P`. The client MUST mark each such item ineligible for inclusion in submit requests, in the same manner and with the same effect as the `runtime_superseded` marking defined in §07 "State and runtime-key rotation": the item MUST NOT be attached to any submit, MUST be retained for user inspection and deletion until its natural `expires_at`, and MUST surface in the chrome state-inspection surface as suspended pending re-consent. Retained `client_only` state items bound to `P` are not transmitted and are unaffected, but MUST remain user-inspectable and deletable. This step does not delete state items; it severs their automatic transmission, so that an item planted under `P` before abandonment - including a credential planted by a `K_runtime`-only attacker (§05) - cannot resume transmission merely because `P` is later re-established.
...
Re-establishment of a previously abandoned `P` does not revive the transmission eligibility of state items suspended under step 5 above. A `request`-mode item suspended at abandonment remains suspended after re-establishment until the publisher issues a fresh `set` operation for its `(namespace, key)` under the re-established identity and the user grants fresh consent (§07); remembered consent recorded before the abandonment does not authorize automatic transmission of a pre-abandonment value. This prevents a credential planted under `P` before abandonment from being auto-attached to submits once the user re-pins `P`, which would otherwise reopen the exposure window that abandonment is invoked to close.

## Publisher profiles across origin replacement

A client that retains trust state across sessions MUST support publisher profiles: a single publisher identity record keyed by `K_publisher.pub`, recognized across origins that the client has verified through the replacement flow below. Cross-session retention without publisher-profile support fragments the user's identity model, because the same publisher reached at a verified successor origin appears as First contact and requires re-verification at every migration. A cross-session client that does not maintain publisher-profile records is not conformant.
...
* the publisher profile has exactly one current origin; verified-pending successors and historical or replaced origins do not create simultaneous current origins;
...
2. apply the full validation pipeline (Stages 1 through 9) to the fetched successor manifest, using the isolated provisional publisher-history state defined below for the history-dependent Stage 8 checks;
...
For check 2, the client MUST create a provisional publisher-history state associated with the pending `(announcing_origin, successor_origin)` migration and isolated from the global publisher history for `K_publisher.pub`. The equal-`issued_at` conflict, anti-downgrade, and runtime-key reuse checks in Stage 8 consult only that provisional state. It is initially empty; the announcing manifest and every other global publisher-history record MUST NOT be treated as a preceding successor publication. A successor manifest that passes Stages 1 through 9 updates the provisional state, so a later refetch or refresh of the same pending successor is checked against earlier provisionally verified successor publications. It does not update the global anti-downgrade floor or runtime-key history before adoption.
...
If all checks pass, the client caches the successor manifest under the provisional migration state and records the successor origin in the publisher profile as a **verified pending successor** for `K_publisher.pub`. A verified pending successor has passed verification but is not yet the current origin of the publisher profile.

While the successor is verified pending, the client MUST NOT auto-navigate to it from the announcing origin's session, MUST NOT migrate cached state to it, and MUST NOT render publisher-controlled content from it under that session. Direct user-initiated navigation to the successor origin's address remains permitted: the cached provisional successor manifest may be used for that direct-navigation session, and the publisher identity's trust state extends to the navigation, preserving continuity. Any successor refresh during that session continues to consult the same isolated provisional history. Direct navigation MUST NOT commit the provisional state to global publisher history, MUST NOT make the successor the publisher profile's current origin, and MUST NOT count as the affirmative confirmation required below.

The verified pending successor transitions to an **adopted** successor - the current origin of the publisher profile, recorded as an Adoption event in publisher history under "Cross-session migration history" below - upon the user-confirmation step defined under "User confirmation" below. On confirmation, the client MUST atomically promote the successor's provisional publication records into global publisher history and record the announcing origin as Replacement. That promotion is the adoption transition, not a second execution of the successor's Stage 8 checks against the announcing manifest; an equal-`issued_at` or shared-runtime-key relationship between the announcing and successor manifests therefore does not retroactively reject an already verified successor. The user-confirmation step is MUST for all announcing trust states (Externally verified, TOFU pinned, and First contact); a verified pending successor never transitions to adopted without it. If the user declines, the client MUST discard the provisional publisher-history state and leave global publisher history unchanged.
...
If the announcing manifest is replaced by a newer manifest (later canary `issued_at`) that omits `migration_pointer`, the client MUST treat the migration as withdrawn. The successor origin previously adopted into the publisher profile remains adopted unless the user explicitly removes it through publisher-history controls; the announcement's withdrawal does not retroactively unbind a successfully verified successor. A verified pending successor that has not yet been adopted is purged from the publisher profile on withdrawal, and its provisional publisher-history state is discarded without changing global publisher history. The cached successor manifest may remain in the manifest cache under the standard caching rules, but it is no longer associated with a pending migration from the announcing origin, and the verified-pending-state behaviors defined under "Successor verification" above no longer apply.

If the announcing manifest is replaced by a newer manifest with a different `migration_pointer` (different `successor_origin`), the client MUST treat the new announcement independently: re-run successor verification for the new successor, prompt the user, and transition the new successor to adopted only on user-confirmation success. Multiple successive migrations are allowed.
...
1. **Automatic chain-depth limit.** "Automatic adoption" in this subsection means following a `migration_pointer` hop within the same navigation flow without prompting the user, relying on a user confirmation already obtained earlier in the flow or on a prior per-pair confirmation that remains in scope under "User confirmation" above (§10:418). The user-confirmation requirement under "User confirmation" above is unaffected: the initial adoption from `A` to `B` always requires user confirmation per §10:407, §10:413, and §10:414. Once `A -> B` has been confirmed, a client MAY automatically adopt at most one further `migration_pointer` hop within the same navigation flow. After that one automatic adoption from `B` to successor `C`, a further `migration_pointer` present on `C`'s manifest pointing to `D` MUST NOT be adopted in the same navigation flow without a new user-confirmation step, evaluated under the trust state of the publisher identity at `C` (which, since `C` adopted under the same `K_publisher.pub`, is the same publisher profile as `A`).
2. **Visited-origin cycle rejection.** The client MUST maintain, for the duration of a single migration-resolution flow, a set `visited_origins` containing the address of every origin visited in that flow, beginning with the announcing origin. Before adopting a successor announced by `migration_pointer.successor_origin.address`, the client MUST check that the address is not already present in `visited_origins`. A successor address already in `visited_origins` is a chain cycle and MUST be rejected as `E_MIGRATION_INVALID` with `details.reason = "chain_cycle"`. The chain-cycle check on a fetched successor's *own* onward `migration_pointer` is evaluated only after that successor has itself passed the full validation pipeline and the publisher/binding continuity checks of "Successor verification" above (§10:401-405): a fetched successor that fails its own verification surfaces `E_MIGRATION_MISMATCH` (`details.mismatch_field = "successor_stage9_failure"`) first, and its onward announcement, hence any `chain_cycle` it would trigger, is not evaluated until it becomes a verified pending successor.
...
A `migration_pointer` whose `successor_origin.address` equals `origin.address` (`details.reason = "self_pointer"`) is already ill-formed at the manifest layer (§06) and is rejected at Stage 5 as part of `migration_pointer` validation; it does not reach the chain check. The other announcement-internal semantic checks - carrier mismatch (`carrier_mismatch`), `announced_at` later than the announcing manifest's `updated` (`announced_at_after_updated`), and the successor address-to-key binding (`successor_key_mismatch`, §06) - are likewise Stage 5 closed-schema cross-field checks on the announcing manifest alone, in the same class as the `origin.not_after` constraints reported as `E_ORIGIN_INVALID` (§11). Only the `chain_cycle` check is evaluated at Stage 9: it consults the per-flow `visited_origins` set, which exists only during migration resolution, and so cannot be computed from the announcing manifest in isolation.
...
The Unavailable state (a failed live manifest fetch) does not override a canary deadline the client can already see has passed. If the client holds a cached manifest for the publisher whose canary `next_expected` is strictly earlier than the sound verified-time lower bound `T_lower` defined below, the client MUST compute Expired (the default render-block and per-session override), not Unavailable, even though the live fetch failed. A missed, client-observable deadline is a publisher commitment failure, not a transport condition, so a coerced or compromised carrier cannot downgrade the Expired render-block to the weaker "Unavailable -> cached content only" row of the table by withholding the manifest fetch. The Unavailable row, and its optional cached-content rendering, apply only when the client holds no cached manifest for the publisher or the held manifest's `next_expected` is not shown definitely past by `T_lower`. See §08 (Unavailable).

The five canary states above are the values of the canary state machine and they are mutually exclusive. Freshness-unverified mode, defined under "Clock reliability and the verified-time reference" above, is not a sixth value of this state machine: it is an orthogonal qualifier on the canary presentation. It applies to a client that cannot establish a reliable current-time reference and therefore cannot compute the time-dependent determinations Fresh, Near-expiration, and Expired. The time-independent determinations - Invalid (structural failure), Unavailable (canary absent or unreachable), and the lower-bound Expired determination available when an expiry timestamp is strictly less than `T_lower` - are unaffected by freshness-unverified mode and continue to govern rendering and chrome per this table. Anti-downgrade enforcement is likewise unaffected. Freshness-unverified mode suppresses only the Fresh, Near-expiration, and (clock-dependent) Expired determinations, replacing them with the explicit chrome indication that the client cannot place the current time within the canary's claimed validity window.
...
A client maintains the anti-downgrade reference `T_verified`, defined as the greatest `canary.issued_at` among all manifests it has verified for the relevant `K_publisher.pub`. `T_verified` is the same quantity as the anti-downgrade floor defined in §08 ("the newest verified `issued_at` previously observed for the same `K_publisher.pub`", §08 anti-downgrade rule and §10 "Anti-downgrade enforcement" above). It is authenticated publisher history, but it is not by itself a sound lower bound on current real time: even with a reliable clock, the future-skew rule permits an accepted `issued_at` to be up to 300 seconds ahead; without a reliable clock, the client cannot enforce that bound at all.

For sound time inference, a client MAY separately maintain `T_lower`. For each manifest verified while the client has a reliable current-time reference and enforces the 300-second future-skew bound on `canary.issued_at`, the candidate lower bound is:

```text
candidate_lower = canary.issued_at - 300 seconds
T_lower = max(previous T_lower, candidate_lower)
```

Only `T_lower` is an authenticated lower bound on current real time. A manifest verified without a reliable current-time reference advances `T_verified` for unconditional anti-downgrade enforcement but MUST NOT advance `T_lower`. When every manifest that has contributed to the current `T_verified` was time-checked, an implementation MAY derive `T_lower` as `T_verified - 300 seconds` rather than storing a second timestamp. Once a non-time-checked manifest advances `T_verified`, the values are no longer derivable from one another and the client retains the last independently established `T_lower`, if any.

At the instant a client successfully verifies a freshly fetched manifest against a reliable current-time reference, that manifest's `issued_at` closely approximates the current real time, within the accepted future-skew bound. It is therefore useful at fetch time for detecting local-clock drift, and its tolerance-adjusted value can advance `T_lower`. Neither `T_verified` nor `T_lower` is a substitute for a current-time reference while the client is offline: neither advances between fetches, so neither can detect that time has passed since the last verified manifest. A client MUST NOT treat either value as evidence that a canary is currently fresh (see "Freshness-unverified mode").

A client maintaining publisher history and a local clock SHOULD compare each freshly verified `issued_at` against its local clock. A divergence beyond the §10 skew tolerance indicates either local-clock drift or an anomalous `issued_at`; the client SHOULD surface a drift advisory inviting clock synchronization, and MUST NOT treat the divergence alone as grounds to reject the manifest, since it cannot distinguish drift from anomaly without an external reference.

### Sound expiration in the lower-bound direction

A client MAY use `T_lower` to determine that an expiry timestamp is definitely past: if `origin.not_after` or a canary `next_expected` is strictly less than `T_lower`, the resource is expired. This determination is sound (no false positives) but incomplete. In the usual case, `T_lower` for a held manifest is no later than that manifest's own `issued_at`, which is by construction earlier than its `next_expected`, so this check does not establish the expiry of the manifest the client currently holds. It can establish that an older retained resource is expired after a later time-checked manifest advances `T_lower`. It does not detect a publisher that stops issuing canaries while the client is offline; that case is handled by freshness-unverified mode.
...
The future-skew bounds on `manifest.updated` and `canary.issued_at` (§10) require an upper bound on current time and cannot be enforced precisely by a client without a reliable clock. Neither the history value `T_verified` nor the lower bound `T_lower` supplies that upper bound, and neither MUST be used as the future-skew reference: doing so rejects legitimate manifests issued after the stale reference - for example, manifests a publisher issues while the client is offline - deadlocking the client against its own retained state.

A client without a reliable clock MUST NOT hard-reject a manifest on a future-skew determination. It MAY apply a loose sanity bound to reject grossly implausible timestamps and SHOULD surface a drift advisory. It advances `T_verified` to the newest verified `issued_at` as usual so that anti-downgrade replay protection is preserved, but it MUST NOT advance `T_lower` from that observation. The residual exposure - an adversary holding `K_publisher_priv` inflating `issued_at` to poison the anti-downgrade floor - is documented as a v1.0 limitation in §00; it requires identity-key compromise, against which a clock-less client has no protocol-provided defense, and an adversary with `K_publisher_priv` can mount more direct attacks regardless.
