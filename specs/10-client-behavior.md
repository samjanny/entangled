...
The Unavailable state (a failed live manifest fetch) does not override a canary deadline the client can already see has passed. If the client holds a cached manifest for the publisher whose canary `next_expected` is at or before the verified current time, the client MUST compute Expired (the default render-block and per-session override), not Unavailable, even though the live fetch failed. This is the time-independent lower-bound Expired determination noted below (the expiry timestamp is at or before the verified-time reference): a missed, client-observable deadline is a publisher commitment failure, not a transport condition, so a coerced or compromised carrier cannot downgrade the Expired render-block to the weaker "Unavailable -> cached content only" row of the table by withholding the manifest fetch. The Unavailable row, and its optional cached-content rendering, apply only when the client holds no cached manifest for the publisher or the held manifest's `next_expected` is still in the future. See §08 (Unavailable).
...
A client MAY maintain a verified-time reference `T_verified`, defined as the greatest `canary.issued_at` among all manifests it has verified for the relevant `K_publisher.pub`. `T_verified` is the same quantity as the anti-downgrade floor defined in §08 ("the newest verified `issued_at` previously observed for the same `K_publisher.pub`", §08 anti-downgrade rule and §10 "Anti-downgrade enforcement" above): the verified-time reference and the anti-downgrade floor are identical values, not parallel concepts. A client maintaining the anti-downgrade floor (as it MUST, per §08 and §10) MAY refer to that same value as `T_verified` for the purposes of this subsection without duplicating storage. `T_verified` is an authenticated lower bound on the current real time: a value an adversary cannot advance without producing a validly signed manifest that itself passes every check, including anti-downgrade.
