...
The verified pending successor transitions to an **adopted** successor — an authorized origin of the publisher profile, recorded as an Adoption event in publisher history under "Cross-session migration history" below — upon the user-confirmation step defined under "User confirmation" below. For announcing trust state **First contact**, the SHOULD-level relaxation of that subsection permits a client that omits the confirmation dialog to treat verification as adoption, provided the chrome migration notice required for First contact is displayed; in that case the transition occurs at verification time.
