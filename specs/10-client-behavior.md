...
If all checks pass, the client records the successor manifest as the cached current manifest for the successor origin under the standard caching rules and records the successor origin in the publisher profile as a **verified pending successor** for `K_publisher.pub`. A verified pending successor has passed verification but is not yet an authorized origin of the publisher profile.
